    ...
    def chat_with_memory(
        ...
        try:
            ...
            memory_results = self.memory.search(
                query=query, user_id=user_id, agent_id=agent_id, limit=5
            )

            logger.info(
                f"Found {len(memory_results.get('results', []))} relevant memories"
            )
            ...
            if store_conversation:
                self._store_conversation(
                    query, response.content, user_id, agent_id, metadata or {}
                )
            ...
            return {
                ...
                "memory_context": (
                    memory_context if memory_results.get("results") else None
                ),
            }
